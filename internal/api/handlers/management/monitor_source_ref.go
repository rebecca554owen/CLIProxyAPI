...
func newMonitorSourceResolver(cfg *config.Config, authManager *coreauth.Manager) *monitorSourceResolver {
	...
	if cfg != nil {
		for index, entry := range cfg.OpenAICompatibility {
			ref := buildProviderMonitorSourceRef("openai", index, providerDisplayName("openai"), entry.Name, false)
			for _, candidate := range collectOpenAIProviderCandidates(entry) {
				if candidate == "" {
					continue
				}
...
func collectOpenAIProviderCandidates(entry config.OpenAICompatibility) []string {
	set := map[string]struct{}{}
	add := func(value string) {
		trimmed := strings.TrimSpace(value)
		if trimmed == "" {
	...
	for _, keyEntry := range entry.APIKeyEntries {
		add(keyEntry.APIKey)
		add(util.HideAPIKey(strings.TrimSpace(keyEntry.APIKey)))
	}
	values := make([]string, 0, len(set))
	for value := range set {
		values = append(values, value)
